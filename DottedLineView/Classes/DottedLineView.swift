...
public class DottedLineView: UIView {
    ...
    private func configurePath(path: UIBezierPath, rect: CGRect) {
        if horizontal {
            ...
            let randomIndex = rect.size.width.truncatingRemainder(dividingBy: (lineWidth * 2))
            
            let drawWidth = rect.size.width - (randomIndex) + lineWidth
            ...
            let center = rect.width * 0.5
             ...
             let randomIndex = rect.size.height.truncatingRemainder(dividingBy: (lineWidth * 2))
            let drawHeight = rect.size.height - (randomIndex) + lineWidth
    ...
    private func configureRoundPath(path: UIBezierPath, rect: CGRect) {
        if horizontal {
            let center = rect.height * 0.5
               let randomIndex = rect.size.width.truncatingRemainder(dividingBy: (lineWidth * 2))
            let drawWidth = rect.size.width - (randomIndex)
            let startPositionX = (rect.size.width - drawWidth) * 0.5 + lineWidth
            ...
            let center = rect.width * 0.5
             let randomIndex = rect.size.height.truncatingRemainder(dividingBy: (lineWidth * 2))
            let drawHeight = rect.size.height - (randomIndex)
            let startPositionY = (rect.size.height - drawHeight) * 0.5 + lineWidth
